...
const JsonNode: React.FC<JsonNodeProps> = ({ value, name, isLast = true, isDarkMode = false }) => {
  ...
  // Khớp chính xác mã màu JSON gốc
  const renderValue = () => {
...
const JsonFormatter: React.FC = () => {
  ...
  const parseInput = (): any | null => {
    ...
    try {
      ...
      try {
        ...
        throw new Error("Input evaluates to a primitive value, not an object/array.");
      } catch (jsErr) {
        setStatus({ message: `❌ Syntax error:\n${jsonErr.message}`, type: 'error' });
        setOutputMode(null);
        return null;
      }
    }
  ...
  const handleCopy = () => {
    ...
    if (outputMode === 'pretty' || outputMode === 'minify') {
      contentToCopy = outputMode === 'pretty' ? JSON.stringify(parsedData, null, 4) : JSON.stringify(parsedData);
      ...
      <div className="max-w-[1400px] mx-auto px-5 py-10">
        ...
        <div className="flex items-center gap-4 mb-[30px]">
          ...
          <div>
            ...
            <p className="text-[14px] text-[#64748b] mt-1">Format, validate, and automatically generate TypeScript interfaces from JSON.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[calc(100vh-150px)] min-h-[700px]">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-[0_10px_25px_-5px_rgba(0,0,0,0.05)] flex flex-col">
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-[18px] font-semibold text-[#0f172a]">1. Paste JSON Here</h3>
              <button
                onClick={handleClear}
                className="px-4 py-2 text-[14px] font-semibold rounded-[10px] bg-[#fee2e2] text-[#ef4444] border border-[#fca5a5] hover:bg-red-100 transition-colors"
              >
                Clear
              </button>
            </div>
            ...
            <textarea
              ...
              placeholder='{"name": "Kiet Junior", "role": "AI Agent"}'
            ...
            {/* ACTION BUTTONS (Khớp layout flex-wrap như cũ) */}
            <div className="flex flex-wrap gap-3 mt-5">
              <button
                onClick={handleFormat}
                className="flex-1 flex justify-center items-center gap-2 py-[14px] px-[20px] bg-[#0d9488] text-white rounded-[10px] text-[14px] font-semibold shadow-[0_4px_12px_rgba(13,148,136,0.2)] hover:bg-[#0f766e] transition-all hover:-translate-y-[2px]"
              >
              ...
              <button
                ...
                className="flex-1 flex justify-center items-center gap-2 py-[14px] px-[20px] bg-[#0f766e] text-white rounded-[10px] text-[14px] font-semibold shadow-[0_4px_12px_rgba(13,148,136,0.2)] hover:bg-[#0b5c56] transition-all hover:-translate-y-[2px]"
              >
              ...
              <button
                onClick={handleTsGen}
                className="w-full basis-full flex justify-center items-center gap-2 py-[14px] px-[20px] bg-gradient-to-br from-[#f59e0b] to-[#d97706] text-white rounded-[10px] text-[14px] font-semibold shadow-[0_4px_12px_rgba(245,158,11,0.2)] hover:-translate-y-[2px] transition-all"
              >
          ...
          <div className="bg-white p-6 rounded-2xl shadow-[0_10px_25px_-5px_rgba(0,0,0,0.05)] flex flex-col">
            <div className="flex justify-between items-center mb-5">
              ...
              <div className="flex gap-2 items-center">
                <button
                  ...
                  className="px-4 py-2 text-[14px] font-semibold rounded-[10px] bg-[#f0fdfa] text-[#0d9488] border border-[#99f6e4] hover:bg-[#ccfbf1] transition-colors"
                >
                  {isDarkMode ? 'Light Theme' : 'Dark Theme'}
                </button>
                <button
                  onClick={handleCopy}
                  className="px-4 py-2 text-[14px] font-semibold rounded-[10px] bg-[#f1f5f9] text-[#334155] border border-[#e2e8f0] hover:bg-[#e2e8f0] hover:border-[#cbd5e1] transition-colors"
                >
                  Copy Result
                </button>
              </div>
            ...
            {/* Hộp Trạng Thái */}
            {status && (
              <div className={`px-4 py-[14px] rounded-[10px] mb-5 text-[13px] font-medium transition-all duration-300 ${status.type === 'success' ? 'bg-[#ecfdf5] text-[#065f46] border border-[#6ee7b7]' : 'bg-[#fef2f2] text-[#991b1b] border border-[#fca5a5]'}`}>
                {status.message}
              </div>
            )}
            ...
            <div className={`flex-1 overflow-auto border rounded-lg transition-colors duration-300 ${isDarkMode ? 'bg-[#0f172a] border-[#334155]' : 'bg-white border-[#e2e8f0]'}`}>
              ...
              <div className={`p-[15px] font-mono text-[14px] whitespace-pre ${isDarkMode ? 'text-[#f8fafc]' : 'text-[#334155]'}`}>
                ...
                {outputMode === 'pretty' && parsedData && (
                  <JsonNode value={parsedData} isDarkMode={isDarkMode} />
                )}

                {outputMode === 'minify' && parsedData && (
